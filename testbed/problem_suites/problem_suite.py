...
from utils.diff import apply_diff, validate_diff
...
class ProblemSuite(ABC):
    ...
    def run_agent_in_sandbox_for_problem(self, sandbox_manager, problem_name, agent_source_code, on_finish):
        ...
        def _on_finish(result):
            if result.get("status") == "success":
                ...
                is_valid_diff, error_msg = validate_diff(diff, sandbox_manager.get_sandbox_temp_dir(sandbox_id))
                if is_valid_diff:
    ...
    def evaluate_solution_diff(self, sandbox_manager, problem_name, solution_diff, on_finish):
        ...
        def on_mount(temp_dir):
            # Copy problem files to the /sandbox directory
            self.copy_problem_files_to_directory(problem_name, temp_dir, include_tests=True)
            
            # Apply the diff
            debug(f"[PROBLEM_SUITE] Applying agent's solution diff to {temp_dir} for problem {problem_name}")
            success, error_msg = apply_diff(solution_diff, temp_dir)
            if not success:
                raise Exception(f"Failed to apply agent's solution diff: {error_msg}")
            debug(f"[PROBLEM_SUITE] Applied agent's solution diff to {temp_dir} for problem {problem_name}")
